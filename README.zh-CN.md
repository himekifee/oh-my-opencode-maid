...
启用后，插件就隐身在模型和你的终端之间。改写成功时，模型吐出的原始草稿会先由 OpenCode 按你配置的口吻重写，再呈现给你；如果改写失败，则会内联显示原始草稿并弹出错误提示。
...
| **持久化** | 改写成功后，被抽走的原文会作为仅供展示的旁路记录存进私有 SQLite 库；之后的模型调用会收到原始草稿而不是可见的角色扮演文本。 |
| **失败路径** | 万一改写失败：插件会把抽走的原始草稿作为普通助手回复内联显示，并弹出错误提示说明正在显示原始草稿。 |
...
当找不到公开的 client 形状时，它还会读一个**私有 SDK 字段**（`client._client`）作为兜底，以便调用 `session.create` / `session.prompt` / `session.delete`。这属于内部接口，OpenCode 升级时可能失效；一旦失效，插件会输出中性的失败文本，而不是暴露无法对应回原始草稿的改写文本。
...
这些记录只供本地展示/恢复路径使用。`experimental.chat.messages.transform` 会把可见的改写后文本留给用户看，但在后续主代理上下文里恢复成原始草稿，避免角色扮演文本污染代理的工作记忆。压缩也不会把原始草稿追加进 `output.context`。改写失败时会直接内联显示原始草稿并弹出错误提示，因此没有需要从后续上下文里移除的改写文本。
...
运行时自测建议在 tmux 里做，把 OpenCode 指到构建好的 `dist/index.js`：发一条普通提示，看看原始草稿会不会一闪而过，确认最终回复确实照着 `roleplay_prompt` 走；再运行 `/maid-rewrite-toggle`，确认改写会立刻关闭和重新打开，同时持久化 `enabled`、显示对应的状态提示，并且不会为了命令结果回复去调用模型。再特意触发一次改写失败，确认抽走的原始草稿会作为助手回复内联显示，并且错误 toast 说明正在显示原始草稿。还要验证落库失败的路径不会暴露未追踪的改写文本，而是在需要时输出中性兜底文字或 `FAILURE`。启用 `dist/tui.js` 后，确认成功改写默认不显示可折叠原文行。当开启 `show_original_draft` 且宿主渲染树可用时，成功改写应该显示一个本地折叠的 `+ Original Draft Content` renderer 行；点击后在本地内联展开，再点一次折叠，而且原文绝不能进入日志、导出、压缩、宿主装饰提示或 overlay。旧版兜底记录不应打开本地弹窗。
